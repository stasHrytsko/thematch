...
- **Frontend:** vanilla JS PWA (`public/`) — без фреймворков и сборки, с Service
  Worker (оффлайн) и manifest (устанавливаемое приложение).
- **Деплой:** Vercel. После переноса PWA в корень **Root Directory = `.`** (корень
  репозитория), не `thematch_pwa`. Конфиг — `vercel.json`.
...
vercel.json         ← конфиг Vercel (includeFiles для services/database/utils)
...
1. Пользователь вводит две даты рождения в `public/` → `app.js` шлёт `POST
...
# фронтенд: открыть public/index.html (или раздать статикой)
...
- переменные окружения, зависимости (`requirements.txt`), конфиг деплоя
