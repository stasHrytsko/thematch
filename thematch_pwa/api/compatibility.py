...
# Accepts two birth dates and returns the full 3-factor compatibility result.
import sys
import os

# Make project root importable when Vercel runs this file from the api/ directory.
...
# Instantiate services once at module level (reused across warm invocations).
...
def _validate_date(date_str: str):
    """Parse DD.MM.YYYY and return a datetime, or raise ValueError."""
...
def compatibility():
    ...
    if not data:
        return _cors(jsonify({"error": "Request body must be JSON"})), 400

    if "date1" not in data or "date2" not in data:
        return _cors(jsonify({"error": "Both 'date1' and 'date2' are required"})), 400
    ...
    # ------------------------------------------------------------------ #
    # Run all three compatibility calculations                             #
    # ------------------------------------------------------------------ #
    ...
    # Weighted total: zodiac 35 %, biorhythm 35 %, numerology 30 %
    ...
    result = {
        ...
        "zodiac": {
            ...
            "sign1_description": _descriptions.ZODIAC_DESCRIPTIONS.get(
                zodiac_details["sign1"], ""
            ),
            "sign2_description": _descriptions.ZODIAC_DESCRIPTIONS.get(
                zodiac_details["sign2"], ""
            ),
            ...
            "elements_emoji": _descriptions.get_emoji(
                zodiac_details["elements_compatibility"]
            ),
        ...
        "biorhythm": {
            ...
            "total_description": _descriptions.get_biorhythm_description(
                "total", bio_score
            ),
            "rhythms": {
                name: {
    ...
    # Optionally persist to history when user_id is supplied
        ...
        try:
            ...
            with Database() as db:
                db.add_check_history(
                    int(user_id), data["date1"], data["date2"], total
                )
